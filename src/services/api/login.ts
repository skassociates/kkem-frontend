...
const tceLogin = async (body: any) => {
  try {
    const response = await axiosInstance.post("/auth/tce/login", {
      ...body,
      type: "tce",
    });
    return response;
...
export const auth = { studentLogin, institutionLogin, adminLogin, tceLogin };
